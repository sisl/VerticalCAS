...
######## OPTIONS #########
ver = 5              # Neural network version
table_ver = 5        # Table Version
hu = 25              # Number of hidden units in each hidden layer in network
saveEvery = 200     # Epoch frequency of saving
totalEpochs = 1000   # Total number of training epochs
trainingDataFiles = "/raid/kjulian3/VertCAS/TrainingData/VertCAS_noResp_newTrans_TrainingData_v%d_%02d.h5" # File format for training data
nnetFiles = "/raid/kjulian3/VertCAS/networks/VertCAS_newTransIntrSpeed_pra%02d_v%d_%dHU.nnet" # File format for .nnet files
modelFiles = "/raid/kjulian3/VertCAS/models/VertCAS_newTransIntrSpeed_pra%d_v%d_%dHU.ckpt" # File format for .nnet files
tbFiles = "/raid/kjulian3/VertCAS/tensorboard/VertCAS_newTransIntrSpeed_pra%d_v%d_%dHU"
##########################

# Custom tensorflow session. Sets up training with either a cpu, gpu, or multiple gpus
def get_session(gpu_ind,gpu_mem_frac=0.4):
...
if len(sys.argv) > 1:
    ...
    f       = h5py.File(trainingDataFiles % (table_ver,pra),'r')
    ...
    # Initialize session
    sess = get_session([gpu])
    sess.run(tf.global_variables_initializer())

    tb_file = tbFiles % (pra, ver, hu)

    writer = tf.summary.FileWriter(tb_file)
    writer.add_graph(sess.graph)

    nnet_file = nnetFiles % (pra, ver, hu)
    model_file = modelFiles % (pra, ver, hu)
