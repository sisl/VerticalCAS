                    ...
                    int split_feature)
{
    ...
    if (NEED_PRINT) {
        ...
        if (isOverlap) {
            printf("split_feature:%d, isOverlap: True, depth:%d, adv_found:%d, max_depth:%f\n", \
                        split_feature, depth,adv_found,max_depth);
        }
        else {
    ...
    if (isOverlap && NEED_FOR_ONE_RUN == 0 && depth < max_depth && !depth_exceeded) {
    ...
    if(NEED_PRINT && !adv_found && depth==0){
        printf("progress: [");
