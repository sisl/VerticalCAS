...
struct NNet
    ...
    function NNet(file,weights,biases,symmetric,numLayers,inputSize,outputSize,maxLayerSize,layerSizes,mins,maxes,means,ranges,numNetworks,numTau,numPra,tauCut,praCut,networkArray)
        return new(file,weights,biases,symmetric,numLayers,inputSize,outputSize,maxLayerSize,layerSizes,mins,maxes,means,ranges,numNetworks,numTau,numPra,tauCut,praCut,networkArray)
    end
end
...
function NNet(file::AbstractString)
    ...
    line = readline(f)
    record = split(line,[',','\n'])
    symmetric = parse(Int32,record[1])
    ...
    close(f)
    NNet(file,weights,biases,symmetric,numLayers,inputSize,outputSize,maxLayerSize,layerSizes,mins,maxes,means,ranges,numNetworks,numTau,numPra,tauCut,praCut,networkArray)
end
